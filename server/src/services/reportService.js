...
import Inspector from "../models/supervisors";
import Submission from "../models/submissions";

export const supervisoryReport = async (
  isTest,
  token,
  from,
  to,
...
const submissionByForm = async (forms, inspectors = [], submissions = []) => {
  ...
  const inspectorSubmissions = inspectorNames.map((inspector) => {
    const inspectorSubmissions = forms.map((form) => {
      ...
      const inspectorItem = inspectors.find((item) => item.name?.toLowerCase() === inspector?.toLowerCase());
      return {
        form,
        inspector,
        county: inspectorItem?.county,
        subCounty: inspectorItem?.subCounty,
        site: inspectorItem?.site,
    ...
    data.county = inspectorItem?.county;
    data.subCounty = inspectorItem?.subCounty;
    data.site = inspectorItem?.site;
...
export const getSubmissions = async (isTest, token, county = "", from, to) => {
  ...
  const submissions = await Promise.all(
    forms.map(async (formItem) => {
      ...
      const list = submissionsList.map((item) => ({
        ...
        inspectors: item?.inspectors_repeat
  ...
  const formNames = forms.map((item) =>
